...
\end{center}
\section{Abstract}
This article describes the development and results of a stochastic SPAYR COVID model that simulates the effect of behavioral changes on a population. The model is based on the SIR model but divides the infectious component into pre-symptomatic, asymptomatic, and symptomatic compartments to accurately model COVID-19. Furthermore, it gives each compartment except removed a parameter that measures the fractional reduction of contacts during an epidemic. We find that the behavior of the non-symptomatic proportion of the population has a large effect on the epidemic. Furthermore, on average asymptomatic's behavioral parameter is the most influential while susceptible's behavioral parameter is the most important parameter for most of our metrics. Our article contributes to a better understanding of COVID-19, and the model's simplicity allows users to easily understand it which can lead to improved behavior during future epidemics.
\tableofcontents
\section{Introduction}
Our motivation for this article began during the COVID-19 epidemic where we observed and imitated the behavior of others. We wanted to understand the impact of these behaviors on ourselves and the population as a whole. Specifically, how do behavioral changes affect the spread of COVID-19? What behavioral changes can contain the spread of COVID-19? To accomplish that we set out to create a model accurate enough to answer our questions. This article contains our research and it is divided into seven sections not including the previous sections and this section. These seven sections are Background and Literature Review, Model Design and Architecture, Implementation, Experimental Setup, Results Analysis, Validation, Discussion, and Conclusion.
\section{Background and Literature Review}
The approach for modeling our problem was heavily influenced by a few key articles. The first is \textit{A simple model for behavior changes in epidemics} which describes how to model behavior changes.  The second is \textit{SEIR modeling of the COVID-19 and its dynamics} which describes the division of a SEIR model. The third is an article on Model Stochasticity which describes the stochastic model language. The fourth is an online article on the Gillespie Algorithm which describes how to model events following a Poisson distribution using Gillespie. These articles which we shall describe give us a strong background for designing our model.
...
\par
To model this we reference \textit{A simple model for behavior changes in epidemics} which describes a modified SIR model. It assumes that susceptible members during an epidemic decrease their rate of contact by a fraction $p, 0 \le p \le 1$, and that infectious members decrease their rate of contact by a fraction $q, 0 \le q \le 1$. This gives us the current deterministic model.
\begin{align*}
...
Our implementation leveraged the python ecosystem, implemented stochastic algorithms, and solved multiple challenges. These actions gave us a functioning program that could be used to run valuable scenarios for analyzing COVID-19.

\subsection{Technologies Used}
The Python ecosystem was essential in implementing our program. Specifically, the Numpy library's multi-dimensional arrays were essential in holding each run's component population over time. On the other hand our calculations employed a mix of the Numpy and Scipy library. Also, Matplotlib helped us in visualizing our simulation with its plots. Last, Pandas was used to export our results in the CSV format. Without the Python ecosystem it would have been much more difficult to complete our implementation.
\begin{itemize}
	\item Numpy==2.3.3
...
\begin{table}[!h]
	...
	\begin{tabular}{|l | l | l |}
		...
		\hline
		skep\_test & Mixed Skeptic Self-Testing Population Scenario     & s=0.55 p=0.50 a=0.50 y=0.30 \\
		\hline
...
\end{table}
\FloatBarrier
For each simulation we have organized its output. Its outputs consists of summary plots, summary metrics, time series arrays of each component, and the array of time for every component. These are listed in detail in Table 7. \par
\begin{table}[!h]
	\centering
...
To best understand our simulation we need to measure which behavioral parameters affect our output the most. Specifically, we will measure the sensitivity of $s$, $p$, $a$, and $y$ for the attack rate, peak time of infections, and peak infection metrics. We will measure the sensitivity by using the calculation method $ \frac{\% change in output}{\% change in input}$. The sensitivity analysis of our behavioral parameters will help us understand which parameter is the most important. \par
\begin{table}[!h]
	\centering
	\begin{tabular}{|l|l|}
		\hline
...
\section{Validation}
In order for our program to be useful it must mirror real world systems. To prove this we have prepared two methods which are Face Validation and Extreme Condition Testing. These methods will ensure that our program accurately simulates the COVID-19 epidemic.
\subsection{Face Validation}
Face validation is a technique that asks if a simulation makes logical sense. We have prepared three runs to face validate our system. The first is default validation which is the same as the default scenario in Results Analysis. The second is realistic validation which is also the same as the realistic scenario in Results Analysis. The third is an isolation validation which tests immediate isolation of infectious components. These runs will help us validate the surface characteristics of our program.
\begin{figure}[H]
	\centering
        \vspace{-4cm}\includegraphics[scale=0.47]{summaryfacevalidation.png}
	\caption{Summary plot of default, realistic, and isolation face validation. Notice that default increases exponentially, peaks, and decreases exponentially. Also, notice that realistic has less infections than the previous plot and is wider. Furthermore, notice how isolation's susceptible population does not decrease.}
\end{figure}
Each example displays behavior that makes logical sense. The first example displays behavior typical of epidemics with an exponential increase, a peak, and a exponential decrease. The second example displays how normal behavioral change, such as the general population decreasing contact and symptomatic persons self-isolating, leads to less infections, a wider peak, and a longer epidemic. Finally, the third example shows how immediate self-isolation for the initial infected population leads to no outbreak of an epidemic. Overall, these runs show that our program's surface characteristics mirror a real world system.
\subsection{Extreme Validation}
Extreme validation is a technique to test a simulation with boundary values to determine whether or not a simulation handles extreme conditions realistically. Three runs have been prepared for extreme validation. The first is a no contact validation which tested immediate isolation for all components. The second is an increased behavior validation which tested behavior parameters above 1.00. The third is an increased infectious population validation which tested large initial values for the infectious population. These runs will ensure that our system mirrors real world systems in extreme conditions.
\begin{figure}[H]
	\centering
	\vspace{-4cm}\includegraphics[scale=0.47]{extremevalidation.png}
	\caption{Summary plot of zero contact, increase contact, and increase symptomatic population extreme validation. Notice how zero contact's susceptible population does not decrease. Also, notice that increased contact's infections increases exponentially, peaks, and decreases exponentially. Furthermore, notice that the increased symptomatic population starts with a large symptomatic population.}
\end{figure}
As you can tell each run displays correct behavior and executes without error. The first example correctly simulates the outcome of zero contact for each component which is a susceptible population that does not decrease. The second example shows that an increased contact rate scenario runs correctly with a relatively rapid rise and fall. Finally, the third and last example displays the correct outcome of an increased symptomatic population which is that the susceptible population immediately decreases and the infectious components increase quickly to mirror the symptomatic component. Overall, these runs show that our program behaves correctly in extreme conditions.
\par
The Face validation and Extreme validation methods have ensured that our program accurately simulates the COVID-19 epidemic. The default, normal, and isolation runs show that our program's surface characteristics mirror a real world system. Furthermore, the no contact, increased behavior, and increased infectious population runs show that our program behaves correctly in extreme conditions. By ensuring the validity of our program we can now safely discuss our results. 
\section{Discussion}
Throughout this document we have discovered several important findings and answered many of our research questions. For example, how do behavioral changes affect the spread of COVID-19? We answer by stating that $a$ on average is the most important parameter for all of our metrics while $s$ is the most important parameter for most of our metrics. Also, the non-symptomatic proportion of infections has a large effect on the epidemic, and scenarios that reduce their impact tend to lead to better outcomes while those that don't lead to worse. Of course lower contact rates especially with $s$ can also lead to good outcomes. Another question is what behavioral changes can contain the spread of COVID-19? We answer that by stating that an epidemic performs the best, meaning low peak infections, high peak times, and low attack rates during a high testing scenario. This project has described much of the behavioral effects on COVID-19.
\par
Our results also have some implications on government policy. Which is that since the non-symptomatic proportion of infections have a large impact on the attack rate and peak infections, the government should prioritize supporting accurate and widespread testing if it is available. Then it should promote in general a cautious attitude towards intimate contact so as to lower the contact rates of the whole population, especially the susceptible population, through public channels. Of course the most controllable method to reduce peak infections and the attack rate is to support the self-isolation of symptomatic individuals because they are easily identified. These actions can significantly reduce the spread and damage of COVID-19.
\par
Overall, this project has produced many valuable insights, implications, and results on the effect of behavioral change on COVID-19.
\section{Conclusion}
Our article contributes to a better understanding of COVID-19 and spreads awareness about the powerful effect behavior changes have on its spread. It described a COVID model that examines the COVID epidemic using the latest knowledge we have on the SARS-CoV-2 virus. Furthermore, unlike other models our model focuses specifically on component based behavior changes and it does this without being too complex to understand for the average person. This makes it a great model for users to understand the impact of behavior change on the spread of an epidemic. This can lead to users changing their behavior during future COVID outbreaks or future epidemics leading to better outcomes.\par
This project has taught us countless lessons on project management, development, and modeling. It taught us the importance of initial research questions in keeping us on track. Furthermore, it taught us how essential it is to discern when tasks need to be dropped for the rest of the project to be completed in time. Also, it showed us how variable the time for software development tasks can be, especially when bugs and errors appear. On the domain side, we were taught the importance in striking a balance between simplicity and complexity when creating models. That a model should only be as complex as it needs to be to answer our research questions. Overall, we have learned many valuable lessons from this project.\par
Though we completed this project there are still some improvements which we did not have time to complete. The most important of which is that some runs are skewed by cycles where the virus randomly and prematurely dies before spreading. This could have been solved by detecting whether a cycle has died prematurely and redoing that cycle until it does not. On the other hand a minor bug is that in some cases the averaging scheme for our plots creates infectious time series that end at one instead of zero, even though our component arrays do not. Some other potential improvements could be by adding more metrics such as R0 which we could not get an accurate measure of. All of these improvements could increase the quality of our model.\par
In the future, we would like to research some of the ideas we thought of during our initial ideation. One of these initial ideas was to model the mutation of COVID strains so we could understand how behavior changes affect the production of SARS-CoV-2 variants. This would've given our article a much more interesting topic though we dropped this as we realized how unlikely and difficult it would be to accurately simulate a virus over such long periods. Though, some time in the future, if we have enough time and knowledge to implement this then I believe it would produce many interesting results and analyses.\par
This article has taken a great effort and I thank all readers who have come this far.
